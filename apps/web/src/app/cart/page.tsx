  ...
  return (
    <Section>
      ...
      <div className="max-w-3xl mb-10 md:mb-12">
        ...
        <h1 className="font-display font-bold uppercase tracking-tight text-white text-[26px] sm:text-[30px] md:text-[36px] leading-tight">
          Ваш заказ
        </h1>
      </div>
      ...
      {rows.length === 0 ? (
        ...
        <div className="grid lg:grid-cols-[1fr_360px] gap-5 md:gap-6">
          ...
          <div className="flex flex-col gap-3">
            {rows.map(({ it, p }) => (
              <article key={it.productId} className="card flex gap-4 items-start">
                <div className="flex-1 min-w-0 flex flex-col gap-3">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <div className="inline-flex items-center rounded-full bg-white/[0.05] border border-white/[0.08]">
                      <button
                        type="button"
                      ...
                      <span className="min-w-[28px] text-center font-semibold text-white">{it.qty}</span>
                      <button
                        type="button"
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[12px] text-white/65 mt-0.5 text-break">
                        {p.installLaborSale.toLocaleString('ru-RU')} ₽ за работу вместо{' '}
                        <span className="line-through text-white/40">
                          {p.installLabor.toLocaleString('ru-RU')} ₽
                        </span>
                      </div>
                    </div>
                  </label>
                ...
                <div className="flex flex-col items-end gap-1 flex-shrink-0">
                  <div className="font-display font-bold text-white text-[18px] md:text-[20px] leading-none">
          ...
          <aside className="flex flex-col gap-4">
            <div className="card-elev flex flex-col gap-4 lg:sticky lg:top-24">
              <div className="flex flex-col gap-2.5">
                <div className="flex justify-between text-[14px]">
                  <span className="text-white/60">Товары ({cart.count})</span>
                  <span className="text-white font-semibold">{cart.subtotal.toLocaleString('ru-RU')} ₽</span>
                </div>
                {cart.laborTotal > 0 && (
                  <>
                    <div className="flex justify-between text-[14px]">
                      <span className="text-white/60">Установка на месте</span>
                      <span className="text-white font-semibold">{cart.laborTotal.toLocaleString('ru-RU')} ₽</span>
                    </div>
                    {cart.laborSavings > 0 && (
                      <div className="flex justify-between text-[12px]">
                        <span className="text-[#22C55E]">Экономия на работе</span>
                        <span className="text-[#22C55E] font-semibold">−{cart.laborSavings.toLocaleString('ru-RU')} ₽</span>
                      </div>
                    )}
                  </>
                )}
              ...
              <div className="pt-4 border-t border-white/10 flex items-end justify-between">
                <span className="text-[12px] font-bold uppercase tracking-[0.18em] text-white/55">Итого</span>
                <span className="font-display font-bold text-white text-[24px] md:text-[28px] leading-none">
              ...
              <div className="p-3 rounded-xl bg-white/[0.03] border border-white/[0.08]">
                ...
                <div className="text-[12px] text-white/60 mt-1.5 text-break">
                  {SITE.address}
                  <br />
                  {SITE.workingHours}
                </div>
              ...
              <p className="text-[11px] text-white/45 text-break">
                После оформления менеджер подтверждает наличие и согласует время.
              ...
              <button
                type="button"
                onClick={cart.clear}
                className="text-[12px] text-white/45 hover:text-white/70 transition-colors mt-1 self-start"
              >
                Очистить корзину
              </button>
            </div>
